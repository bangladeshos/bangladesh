...
def check_valid_yaml() -> bool:
    ...
    """
    try:
        logger.info(f"Checking if bangladeshos-here is still valid YAML")
        with open(CONTRIBUTORS_FILE_PATH, "r") as f:
            yaml.safe_load(f)
        return True
    except yaml.YAMLError:
        logger.error(f"Error: bangladeshos-here does not contain valid YAML.")
        return False
...
def make_final_comment(user: str, errors: []) -> str:
    ...
    if errors:
        ...
        result += (
            ...
            "[Contributing Guidelines](https://github.com/bangladeshos/bangladeshos.github.io/blob/main/.github/CONTRIBUTING.md#guidelines)"
        )
    else:
